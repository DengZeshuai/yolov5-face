...
def get_transform(center, scale, output_size, rot=0):
    ...
    h = 200 * scale
    t = np.zeros((3, 3))
    t[0, 0] = float(output_size[1]) / h # 沿x轴缩放
    t[1, 1] = float(output_size[0]) / h # 沿y轴缩放
    t[0, 2] = output_size[1] * (-float(center[0]) / h + .5) # 沿x轴平移
    t[1, 2] = output_size[0] * (-float(center[1]) / h + .5) # 沿y轴平移
    t[2, 2] = 1
    if not rot == 0:
