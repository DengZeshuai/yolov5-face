...
class conv_bn_relu_maxpool(nn.Module):
    def __init__(self, c1, c2, kernel_size=3,padding=1,dilation=1):  # ch_in, ch_out
        super(conv_bn_relu_maxpool, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(c1, c2, kernel_size=kernel_size, stride=2, padding=padding, dilation=dilation,bias=False),
            nn.BatchNorm2d(c2),
...
class Shuffle_Block(nn.Module):
    def __init__(self, inp, oup, stride,kernel_size=3,padding=1,dilation=1):
        ...
        if self.stride > 1:
            self.branch1 = nn.Sequential(
                self.depthwise_conv(inp, inp, kernel_size=kernel_size, stride=self.stride, padding=padding,dilation=dilation),
                nn.BatchNorm2d(inp),
                nn.Conv2d(inp, branch_features, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(branch_features),
                nn.ReLU(inplace=True),
        ...
        self.branch2 = nn.Sequential(
            ...
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
            self.depthwise_conv(branch_features, branch_features, kernel_size=kernel_size, stride=self.stride, padding=padding,dilation=dilation),
            nn.BatchNorm2d(branch_features),
            nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
